#Search Engine Program in Python
...
#1. Find the first link tag in input string
#   input: <div id="top_bin"><div id="top_content" class="width960">'
#         '<div class="udacity float-left"><a href="http://udacity.com">'......
#
#   output: http://udacity.com

def get_next_target(page):
    start_link = page.find('<a href=')

    #What if there is no link tag in input string?
    if start_link == -1:
        return None,0
    #if there is link tag, it behaves normally
...
#2. Repeat the first step to gain all of the link tags
#   input: a seed page
#   output: list of all link tags in that page
...
# Union function is needed for crawling process later
...
# add_page_to_index function to make indexing
# input: index, url(String), content(String)
# output: update the index to include all of the world occurences found
# in the page content by adding the url to the word's associated url list
...
#3. Crawling Process
#   input: a seed page
#   output: list of all the urls that can be reached by following links,
#   starting from the seed page


def crawl_web(seed):
    tocrawl = [seed]   #tocrawl = ['.../index.html']
    crawled = []
    index = {}
    while tocrawl:
        page = tocrawl.pop()    #tocrawl = [];
                                #page = ['.../index.html']
        if page not in crawled: #we have to make sure
                                #that the page hasn't be crawled before
            content = get_page(page)
            add_page_to_index(index, page, content)
            union(tocrawl, get_all_links(content))  #tocrawl = ['/crawling.html', '/walking.html', '/flying.html']
            crawled.append(page)                    #crawled = ['/index.html']
    return index
            

#4. Lookup
#   find the keyword's url
...
print index
print lookup(index, 'Programming')
